...
def levelTWO(screen: pygame.Surface, tile_map: list[str], jukebox) -> None:
    ...
    idle_frames = [
        pygame.image.load(f"Death Knight Idle/idle_frame_{i}.png") for i in range(1, 9)
    ]
    running_raw = [
    ...
    ammo_counts["water_gun"] = 25
    ammo_counts["water_balloon"] = 5
    projectiles            = []
    weapon_cooldown        = 0
    shop_cooldown          = 0
    ...
    kelp_pool = [
        {"bx": x, "phase": random.random() * 6.28, "h": 28 + random.random() * 38}
    ...
    while running:
        ...
        for b in bubble_pool:
            ...
            if -10 < vx_b < WIDTH + 10:
                bs = pygame.Surface(
                    (int(b["r"] * 2 + 2), int(b["r"] * 2 + 2)), pygame.SRCALPHA
                )
                pygame.draw.circle(
                    bs, (120, 200, 180, 55), (int(b["r"]), int(b["r"])), int(b["r"])
                )
                pygame.draw.circle(
                    bs, (160, 230, 210, 110), (int(b["r"]), int(b["r"])), int(b["r"]), 1
                )
