...
# le bron also loves mygame
def levelFIVE(screen: pygame.Surface, tile_map: list[str], jukebox) -> None:
    global clock
    ...
    idle_frames = [
        pygame.image.load(
            constants.resource_path(f"Death Knight Idle/idle_frame_{i}.png")
        )
        for i in range(1, 9)
    ]
    running_raw = [
        ...
        pygame.image.load(
            constants.resource_path("Death Knight Running/frame_r_2.png")
        ),
    ...
    shopkeeper_img = pygame.image.load(
        constants.resource_path("helper/mr shopKeeper.png")
    )
    shopkeeper_img = pygame.transform.scale(shopkeeper_img, (80, 100))
    spear_frames_raw = [
        pygame.image.load(constants.resource_path(f"Death Knight Spear/s{i}.png"))
        for i in range(1, 5)
    ]
    spear_h = idle_frames[0].get_height()
    spear_w_scale = [
        int(f.get_width() * spear_h / f.get_height()) for f in spear_frames_raw
    ]
    spear_frames = [
    ...
    while running:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_f and input_allowed:
                    ...
                    for lorey in lore_drop:
                        if not lorey["collected"] and player_rect.inflate(
                            40, 20
                        ).colliderect(lorey["rect"]):
                ...
                if (
                    ...
                    and input_allowed
                    and constants.player_flasks > 0
                    and player_hp < constants.PLAYER_MAX_HP
                ):
        ...
        for elev in elevators:
            if (
                elev["rect"].top - SNAP_TOLERANCE
                <= player_rect.bottom
                <= elev["rect"].top + SNAP_TOLERANCE
                and player_rect.right > elev["rect"].left
                and player_rect.left < elev["rect"].right
                and player_vel_y >= 0
            ):
                riding_elev = elev
                break
        ...
        static_solids = (
            ...
            + [
                d["rect"]
                for d in animated_doors
                if d["rect"].height > 0
                and (not d["open"] or d["offset_y"] < d["max_open"])
            ]
        )
        ...
        horizontal_solids = static_solids + [
            e["rect"] for e in elevators if e is not riding_elev
        ]
                    ...
                    for static_floor in static_solids:
                        if player_rect.colliderect(static_floor):
                            death_screen.show_death_screen(
                                screen=screen,
                                clock=clock,
                                restart_func=lambda: levelFIVE(screen, tile_map, jukebox),
                                jukebox=jukebox
                            )
                            return
        ...
        for plat in all_solids:
            if player_rect.colliderect(plat):
                if player_vel_y >= 0:
                    if (
                        player_rect.bottom - player_vel_y * dt
                        <= plat.top + SNAP_TOLERANCE
                    ):
                        player_rect.bottom = plat.top
                    ...
                    if (
                        player_rect.top - player_vel_y * dt
                        >= plat.bottom - SNAP_TOLERANCE
                    ):
        ...
        if riding_elev is not None:
            if (
                player_rect.right > riding_elev["rect"].left
                and player_rect.left < riding_elev["rect"].right
            ):
                is_grounded = True
                can_double_jump = True
            else:
        ...
        enemy_result = enemies_module.update_enemies(
            ...
            screen,
            clock,
            lambda: levelFIVE(screen, tile_map, jukebox),
            projectiles,
            dt,
        )
        ...
        for plat in platforms:
            ...
            pygame.draw.rect(
                screen, GROUND_DIRT, (vx, vy + 8, plat.width, plat.height - 8)
            )
        ...
        for door in breakable_doors:
            ...
            if (
                vx + door["rect"].width < 0
                or vx > WIDTH
                or vy + door["rect"].height < 0
                or vy > HEIGHT
            ):
                continue
            color = COLOR_DOOR_DAMAGED if door["hp"] < 3 else COLOR_DOOR_INTACT
            pygame.draw.rect(
                screen, color, (vx, vy, door["rect"].width, door["rect"].height)
            )
            pygame.draw.rect(
                screen,
                (220, 160, 80),
                (vx, vy, door["rect"].width, door["rect"].height),
                3,
            )
            for i in range(door["hp"]):
                pygame.draw.circle(
                    screen, (255, 220, 80), (int(vx + 10 + i * 14), vy + 8), 5
                )
        ...
        for clip in paperclips:
            ...
            if (
                vx + clip["rect"].width < 0
                or vx > WIDTH
                or vy + clip["rect"].height < 0
                or vy > HEIGHT
            ):
                continue
        ...
        for lorey in lore_drop:
            ...
            if (
                vx + lorey["rect"].width < 0
                or vx > WIDTH
                or vy + lorey["rect"].height < 0
                or vy > HEIGHT
            ):
                continue
            ...
            if not lorey["collected"]:
                ...
                pygame.draw.circle(
                    alpha_surf, glow_col, (radius + 2, radius + 2), radius + 2
                )
        ...
        for elev in elevators:
            ...
            pygame.draw.rect(
                screen,
                COLOR_ELEVATOR,
                (vx, vy, elev["rect"].width, elev["rect"].height),
                border_radius=4,
            )
            pygame.draw.rect(
                screen,
        ...
        screen.blit(
            frame, (player_rect.x - camera_x - 24, player_rect.y - camera_y - 44)
        )
        ...
        for lorey in lore_drop:
            ...
            if not lorey["collected"]:
                ...
                screen.blit(
                    q_surf,
                    (cx - q_surf.get_width() // 2, cy - q_surf.get_height() // 2),
                )
        ...
        hp_color = (
            (56, 200, 122)
            if hp_ratio > 0.4
            else (220, 140, 40) if hp_ratio > 0.2 else (210, 60, 60)
        )
        pygame.draw.rect(
            screen,
            (20, 45, 30),
            (bar_x, bar_y, bar_w, bar_h),
            border_radius=max(1, int(4 * hs)),
        )
        pygame.draw.rect(
            screen,
            hp_color,
        ...
        hp_val = ui_font_xs.render(
            f"{player_hp} / {constants.PLAYER_MAX_HP}", True, (100, 220, 160)
        )
        ...
        screen.blit(
            flask_surf,
            (
                PNL_X + PNL_W - flask_surf.get_width() - int(8 * hs),
                PNL_Y + int(38 * hs),
            ),
        )
        ...
        for i in range(min(clips_total, 5)):
            ...
            pygame.draw.circle(
                screen,
                dot_col,
                (CL_X + int(14 * hs) + i * int(14 * hs), CL_Y + int(14 * hs)),
                max(1, int(4 * hs)),
            )
        ...
        screen.blit(
            p_surf, (CL_X + CL_W - p_surf.get_width() - int(8 * hs), CL_Y + int(3 * hs))
        )
        ...
        for wi, wk in enumerate(weapon_list):
            ...
            sy = (
                HEIGHT
                - int(38 * hs)
                - (len(weapon_list) - 1 - wi) * (slot_h + slot_gap)
            )
