...
def name_entry(screen, clock):
    ...
    while running:
        ...
        screen.blit(p, (constants.WIDTH // 2 - p.get_width() // 2, constants.HEIGHT // 2 - 80))
        n = font.render(name + "|", True, constants.WHITE)
        screen.blit(n, (constants.WIDTH // 2 - n.get_width() // 2, constants.HEIGHT // 2 - 20))

        inst = sub.render("ESC to skip - Backspace to delete", True, (80, 130, 150))
        screen.blit(inst, (constants.WIDTH // 2 - inst.get_width() // 2, constants.HEIGHT // 2 + 40))
...
def show_high_scores(screen, clock):
    ...
    while running:
        ...
        if not scores:
            ...
            screen.blit(empty, (constants.WIDTH // 2 - empty.get_width() // 2, constants.HEIGHT // 2 - 20))
        else:
            headers = ["#", "NAME", "SCORE", "TIME", "DATE"]
            col_x = [60, 110, constants.WIDTH // 2 + 40, constants.WIDTH // 2 + 180, constants.WIDTH // 2 + 280]
        ...
        screen.blit(back, (constants.WIDTH // 2 - back.get_width() // 2, constants.HEIGHT - 50))
