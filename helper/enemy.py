...
def _get_punchy_squid_frames():
    global _punchy_squid_frames
    if _punchy_squid_frames is None:
        base = os.path.join(os.path.dirname(__file__), "..")
        folder = os.path.join(base, "Punchy Squid Idle")
        names = sorted(
            f for f in os.listdir(folder) if f.endswith(".png")
        )
        _punchy_squid_frames = [
...
def render_enemies(screen, enemies, camera_x, camera_y):
    for enemy in enemies:
        ...
        if enemy.get("type") == "punchy_squid":
            frames = _get_punchy_squid_frames()
